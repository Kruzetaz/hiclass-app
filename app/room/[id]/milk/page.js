...
export default function MilkPage() {
  ...
  function clearAll() { setRecords({}); setSaved(false) }
  ...
  const pct = students.length > 0 ? Math.round((receivedCount / students.length) * 100) : 0
      ...
      <div style={{ minHeight: '100vh', background: '#f4f5f7', fontFamily: 'Noto Sans Thai, sans-serif' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1a1f2e 0%, #252d42 100%)', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }}>
          <div style={{ padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: '1px solid #252b3b' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#f1f5f9' }}>🥛 ดื่มนม</div>
          ...
          <div style={{ padding: '14px 20px 16px', display: 'flex', gap: 20, alignItems: 'center' }}>
            {[
              { label: 'รับนมแล้ว',  val: receivedCount,                  icon: '🥛', color: '#8b5cf6' },
              { label: 'ยังไม่รับ',  val: students.length - receivedCount, icon: '⬜', color: '#f87171' },
        ...
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '20px 16px' }}>
          ...
          {students.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
              {students.map((s, i) => {
                const received = records[s.id]
                return (
                  <div key={s.id} className="stu-card"
                    onClick={() => toggle(s.id)}
                    style={{
                      borderColor: received ? '#8b5cf6' : '#e8eaed',
                      background: received ? '#f5f3ff' : '#ffffff',
                    ...
                    <div style={{
                      ...
                      background: received ? '#8b5cf6' : (s.gender === 'female' ? '#f472b6' : s.gender === 'male' ? '#60a5fa' : AVATAR_COLORS[i % AVATAR_COLORS.length]),
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      fontSize: 13, fontWeight: 700, color: '#fff',
                      transition: 'background 0.2s',
                    }}>
                      {received ? '🥛' : s.full_name[0]}
                    </div>

                    {/* ชื่อ */}
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: received ? '#5b21b6' : '#111827', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                        {s.full_name}
                      </div>
                    ...
                    <div style={{
                      ...
                      background: received ? '#ede9fe' : '#f3f4f6',
                      color: received ? '#5b21b6' : '#9ca3af',
                      transition: 'all 0.2s', flexShrink: 0,
                    }}>
                      {received ? '✓ รับแล้ว' : 'ยังไม่รับ'}
                    </div>
                  </div>
                )
              })}
