...
export default function FoodPage() {
  ...
  async function loadData() {
    ...
    const { data: recData } = await supabase
      .from('food_records').select('*').eq('room_id', roomId).eq('date', selectedDate)
    const recMap = {}
      ...
      <div style={{background: 'linear-gradient(135deg, #e07b39 0%, #f5a25d 100%)'}}>
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">🍱 อาหารกลางวัน</h1>
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-orange-100 text-center">
            <p className="text-3xl font-bold" style={{color: '#4CAF50'}}>{receivedCount}</p>
            <p className="text-xs mt-1" style={{color: '#a0856c'}}>รับอาหารแล้ว</p>
          </div>
          <div className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-orange-100 text-center">
        ...
        <div className="flex gap-2 mb-4">
          <button onClick={() => { const m = {}; students.forEach(s => m[s.id] = true); setRecords(m); setSaved(false) }}
            ...
            style={{background: '#4CAF50'}}>✓ เลือกทั้งหมด</button>
        ...
        <div className="flex flex-col gap-2 mb-4">
          {students.map((s, i) => (
            <div key={s.id}
              onClick={() => toggle(s.id)}
              className="bg-white rounded-2xl px-4 py-3 shadow-sm border flex items-center gap-3 cursor-pointer transition"
              style={{borderColor: records[s.id] ? '#4CAF50' : '#f0f0f0'}}>
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-lg"
                style={{background: records[s.id] ? '#E8F5E9' : '#f5f5f5'}}>
                {records[s.id] ? '✅' : '⬜'}
              </div>
            </div>
          ))}
