      ...
      <div style={{background: 'linear-gradient(135deg, #e07b39 0%, #f5a25d 100%)'}}>
        <div className="max-w-lg mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-white">🪥 แปรงฟัน</h1>
      ...
      <div className="max-w-lg mx-auto px-4 py-4">
        ...
        <div className="flex gap-3 mb-4">
          <div className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-orange-100 text-center">
            <p className="text-3xl font-bold" style={{color: '#2196F3'}}>{doneCount}</p>
            <p className="text-xs mt-1" style={{color: '#a0856c'}}>แปรงแล้ว</p>
          </div>
          <div className="flex-1 bg-white rounded-2xl p-4 shadow-sm border border-orange-100 text-center">
            <p className="text-3xl font-bold" style={{color: '#F44336'}}>{students.length - doneCount}</p>
            <p className="text-xs mt-1" style={{color: '#a0856c'}}>ยังไม่แปรง</p>
          </div>
        </div>
        <div className="flex gap-2 mb-4">
          <button onClick={() => { const m = {}; students.forEach(s => m[s.id] = true); setRecords(m); setSaved(false) }}
            className="flex-1 py-2 rounded-xl text-sm font-semibold text-white"
            style={{background: '#2196F3'}}>✓ เลือกทั้งหมด</button>
        ...
        <div className="flex flex-col gap-2 mb-4">
          {students.map((s, i) => (
            <div key={s.id} onClick={() => toggle(s.id)}
              ...
              style={{borderColor: records[s.id] ? '#2196F3' : '#f0f0f0'}}>
              ...
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-lg"
                style={{background: records[s.id] ? '#E3F2FD' : '#f5f5f5'}}>
                {records[s.id] ? '🪥' : '⬜'}
              </div>
            </div>
          ))}
        ...
        {students.length > 0 && (
          <button onClick={saveRecords} disabled={saving}
            ...
            style={{background: saved ? '#2196F3' : 'linear-gradient(135deg, #e07b39, #f5a25d)'}}>
